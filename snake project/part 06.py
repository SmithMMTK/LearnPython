...
def displayCoordinates(stdscr, x, y, max_x, max_y,turn,length):
    ...
    stringToDisplay = f"({x}, {y}), turn {turn}, tail length {length} :"
...
def paintPrompt(stdscr, x, y, promptLength,turnTime,turnHistory):
...
def main(stdscr):
    ...
    while True:
        ...
        if firstTime:
            ...
            # Get key input and check if it is 'q'
            key = stdscr.getch()
            # Detect 'q' key input at the first time
            if key == ord('q'):                
                return 'QUIT'
            ...
            displayCoordinates(stdscr, x, y,max_x,max_y,turn, promptLength) 
        
        
        ## Display the prompt ##

        ## if paintPrompt returns False, break the loop
        if not (paintPrompt(stdscr, x, y, promptLength,turnTime,turnHistory)):
            #stdscr.clear()
            stdscr.timeout(-1) 
