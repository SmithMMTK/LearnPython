...
def main(stdscr):
    ...
    while True:
        ...
        if (turnFruit == FRUITTURN) and (not fruiteMode):
            ...
            while True:
                # Random position of the fruit in the game within the playable area
                fruitPosition = [random.randint(0,max_y-2),random.randint(0,max_x-1)]
        ...
        # Check if the prompt hit the fruit
        if fruiteMode and (y == fruitPosition[0]) and (x == fruitPosition[1]):
            fruiteMode = False
            turnFruit = -1
            promptLength += 1
            turn = -1
